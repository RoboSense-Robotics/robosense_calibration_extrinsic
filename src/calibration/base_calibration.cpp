...
bool BaseCalibration::loadRegistrationConfig(const std::string _config_path) {
  ...
  ndt_param_.leaf_size = ndt_config["LeafSize"].as<std::vector<double>>();
...
{
  // -----------------设置ndt相关参数-----------------------
  approximate_voxel_filter_.setLeafSize(ndt_param_.leaf_size[0], ndt_param_.leaf_size[1],
                                        ndt_param_.leaf_size[2]);
...
std::vector<double> BaseCalibration::executeCalibration(std::vector<pcl::PointCloud<pcl::PointXYZI>> & _straight_cloud_vec,
...
pcl::PointCloud<pcl::PointXYZI>::Ptr BaseCalibration::getNDTMatch(
    const pcl::PointCloud<pcl::PointXYZI>::Ptr& _source_cloud,
  ...
  approximate_voxel_filter_.setInputCloud(_source_cloud);
  approximate_voxel_filter_.filter(*filter_cloud);
  pcl::PointCloud<pcl::PointXYZI>::Ptr transformed_cloud(new pcl::PointCloud<pcl::PointXYZI>);
  ndt_.setInputSource(filter_cloud);
...
Eigen::Matrix4d BaseCalibration::getPointCloudFirstToLastRegistration(std::vector<pcl::PointCloud<pcl::PointXYZI>> & _straight_cloud_vec)
{
  ...
  if (save_path_.length()>0)
  {
    ...
    pcl::io::savePCDFileASCII(cur_path, *last_cloud);
  }
  ...
  for (size_t i = 1; i<_straight_cloud_vec.size();i++)
  {
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr trans_cur_cloud = getNDTMatch(cur_cloud, last_cloud, cur_to_start_tf, cur_to_first_tf);
